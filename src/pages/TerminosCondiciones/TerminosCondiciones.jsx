...
const SLUG = 'terminos-condiciones';
...
const TerminosCondiciones = () => {
  ...
  const handleSave = async (values) => {
    ...
    try {
      ...
      message.success('Términos y Condiciones actualizados correctamente');
    } catch {
      ...
      >
        <Card title="Encabezado" style={{ marginBottom: 24 }}>
          <Form.Item label="Título principal" name="title">
            <Input placeholder="TÉRMINOS Y CONDICIONES" />
          </Form.Item>
        </Card>
                ...
                {fields.map(({ key, name }) => (
                  ...
                  >
                    <Form.Item label="Título" name={[name, 'title']}>
                      <Input placeholder="Ej. 1. Objeto del sitio web" />
                    </Form.Item>
